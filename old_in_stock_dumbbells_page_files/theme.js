			...
			if (url) {
				...
				$.ajax({
					...
					success: function(response) {
						...
						theme.PostAjaxModal.initialize($container.find('.page-portfolios'));
						porto_init();
						theme.PostFilter.initialize($container.find('.portfolio-filter'), 'portfolio');
					}
				}).always(function() {
			...
			if (url) {
				...
				$.ajax({
					...
					success: function(response) {
						...
						theme.PostAjaxModal.initialize($container.find('.page-members'), 'member');
						porto_init();
						theme.PostFilter.initialize($container.find('.member-filter'), 'member');
					}
				}).always(function() {
						$container.removeClass('porto-ajax-loading');
					});
			...
			if (url) {
				...
				$.ajax({
					...
					success: function(response) {
						...
						$container.html($response_container.html());
						porto_init();
						theme.PostFilter.initialize($container.find('.faq-filter'), 'faq');
					}
				}).always(function() {
						$container.removeClass('porto-ajax-loading');
					});
...
(function (theme, $, undefined) {
	...
	/* Content Box */
	if ($('.porto-ultimate-content-box').length) {
...
/*
     FILE ARCHIVED ON 14:57:31 Feb 02, 2020 AND RETRIEVED FROM THE
...
playback timings (ms):
  ...
  PetaboxLoader3.datanode: 42.454 (4)
  esindex: 0.015
  LoadShardBlock: 2298.817 (3)
  RedisCDXSource: 136.319
  CDXLines.iter: 14.062 (3)
  exclusion.robots.policy: 0.177
  load_resource: 83.954
  captures_list: 2452.157
  PetaboxLoader3.resolve: 2325.784 (2)
*/